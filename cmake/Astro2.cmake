...
function(astro_mode_map token output)
   string(ASCII 27 Esc)
   macro(set_mode_code code)
      foreach(var ${ARGN})
         set(Mode_${var} "${code}")
      endforeach()
   endmacro()

   set_mode_code("${Esc}[0m"  reset               R   0)
   set_mode_code("${Esc}[1m"  bold                B   1)
   set_mode_code("${Esc}[2m"  Dim                 D   2)
   set_mode_code("${Esc}[3m"  italic              I   3)
   set_mode_code("${Esc}[4m"  underline           U   4)
   set_mode_code("${Esc}[5m"  blink               Bl  5)
   set_mode_code("${Esc}[7m"  reverse             Rv  7)
   set_mode_code("${Esc}[8m"  hidden              H   8)
   set_mode_code("${Esc}[9m"  strikethrough       S   9)
   set_mode_code("${Esc}[22m" reset_dim_bold      rDB 22)
   set_mode_code("${Esc}[23m" reset_italic        rI  23)
   set_mode_code("${Esc}[24m" reset_underline     rU  24)
   set_mode_code("${Esc}[25m" reset_blink         rBl 25)
   set_mode_code("${Esc}[27m" reset_reverse       Rv  27)
   set_mode_code("${Esc}[28m" reset_hidden        rH  28)
   set_mode_code("${Esc}[29m" reset_strikethrough rS  29)

   set(${output} "${Mode_${token}}" PARENT_SCOPE)
endfunction(astro_mode_map)
...
function(astro_color_map token output)
   ...
   #string(REGEX MATCH "id_([0-9]+)" msg "${CMAKE_MATCH_0}")
   string(FIND ${token} "id_" idx)
   message(STATUS "IDX: ${idx} ${token}")
   #string(REGEX REPLACE "${Esc}\[(1;)?[0-9]*m" "" clean_msg "${msg}")

   set_color_code("${Esc}[30m" black   bk 30)
   set_color_code("${Esc}[31m" red     r  31)
   set_color_code("${Esc}[32m" green   g  32)
   set_color_code("${Esc}[33m" yellow  y  33)
   set_color_code("${Esc}[34m" blue    b  34)
   set_color_code("${Esc}[35m" magenta m  35)
   set_color_code("${Esc}[36m" cyan    c  36)
   set_color_code("${Esc}[37m" white   w  37)
   set_color_code("${Esc}[39m" reset   rs 39)

   set(${output} "${Color_${token}}" PARENT_SCOPE)
endfunction(astro_color_map)

function(astro_bright_mode token output)
   astro_color_map("${token}" color_code)
   math(EXPR bright_code "${color_code}+60")
   set(${output} "${bright_code}" PARENT_SCOPE)
endfunction(astro_bright_mode)

function(astro_fg_mode token output)
   astro_color_map("${token}" color_code)
   set(${output} "${color_code}" PARENT_SCOPE)
endfunction(astro_fg_mode)

function(astro_bg_mode token output)
   astro_color_map("${token}" color_code)
   math(EXPR bg_code "${color_code}+10")
   set(${output} "${bg_code}" PARENT_SCOPE)
endfunction(astro_bg_mode)
