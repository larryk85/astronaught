...
namespace versa::literals {
   consteval static inline versa::cryptid::uint128_t operator""_ui128(const char* x) {
      uint64_t high = 0;
      uint64_t low = 0;
      bool high_set = true;
      int shift = 0;
   ...
   consteval static inline versa::cryptid::low_value operator""_low(unsigned long long v) noexcept {
      return {static_cast<std::uint64_t>(v)};
   }

   consteval static inline versa::cryptid::high_value operator""_high(unsigned long long v) noexcept {
      return {static_cast<std::uint64_t>(v)};
   }
}