...
namespace versa::util {
   ...
   template <std::size_t N>
   struct frozen::string {
      using self_t = frozen::string<N>;

      template <std::size_t M>
      using templ_t = frozen::string<M>;

      constexpr static inline auto size_v = N-1;

      frozen::string() = delete;
      frozen::string(const frozen::string&) = default;
      frozen::string(frozen::string&&) = default;
      frozen::string& operator=(const frozen::string&) = default;
      frozen::string& operator=(frozen::string&&) = default;

      constexpr inline frozen::string(const char (&str)[N]) noexcept {
         std::copy_n(str, size_v, _data);
      }

      constexpr inline frozen::string(const std::array<char, N>& arr) noexcept {
      ...
      template <std::size_t M>
      VERSA_CT_CONST inline std::strong_ordering compare(frozen::string<M> other) const noexcept {
      ...
      template <std::size_t M>
      VERSA_CT_CONST inline std::strong_ordering operator<=>(frozen::string<M> other) const noexcept {
         return compare(other);
      }

      template <std::size_t M>
      VERSA_CT_CONST inline bool operator==(frozen::string<M> other) const noexcept {
         if constexpr (size_v != decltype(other)::size_v) {
            return false;
         } else if constexpr (size_v == decltype(other)::size_v) {
            for (std::size_t i = 0; i < size_v; ++i) {
      ...
      template <std::size_t M>
      VERSA_CT_CONST inline bool starts_with(frozen::string<M> other) const noexcept {
      ...
      template <std::size_t M>
      VERSA_CT_CONST inline bool ends_with(frozen::string<M> other) const noexcept {
         constexpr auto osz = decltype(other)::size_v;
         if constexpr (size_v < osz) {
            return false;
         } else {
      ...
      template <std::size_t M>
      VERSA_CT_CONST inline auto concat(frozen::string<M> other) const noexcept {
      ...
      template <std::size_t M>
      VERSA_CT_CONST inline decltype(auto) operator+(frozen::string<M> other) const noexcept {
         return concat(other);
      }
   ...
   template <std::size_t N>
   static inline std::string to_string(frozen::string<N> fs) noexcept {
   ...
   namespace detail {
      template <std::integral I, frozen::string FS>
